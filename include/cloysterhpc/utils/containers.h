 ...
 * @brief A C++20 concept to check if a type is a key-value associative container.
 *
 ...
 * and a `find()` member function that takes a key and returns a `const_iterator`.
 * This allows for compile-time checking and function overloading based on container properties.
 *
...
/**
 * @brief Finds a value by key in an associative container and returns it as an optional.
 *
 * This function provides a safe and idiomatic way to search for an element by key.
 * If the key is found, it returns a `std::optional` containing the corresponding value.
 * If the key is not found, it returns a `std::nullopt`. This prevents the need for
 * manual checks against `container.end()` and avoids returning default-constructed values.
 *
 * @tparam Container The type of the associative container.
 * @tparam Key The type of the key.
 * @param container The container to search within.
 * @param key The key to look for.
 * @return `std::optional<typename Container::mapped_type>` containing the value if found,
 * or `std::nullopt` otherwise.
 */
template <typename Container, typename Key>
    requires AssociativeContainer<Container, Key>
std::optional<typename Container::mapped_type>
findOpt(const Container& container, const Key& key) noexcept
{
...
/**
 * @brief Finds a value by key in an associative container, throwing an exception if not found.
 *
 * This function is a convenience wrapper around `findOpt`. It provides "fail-fast"
 * behavior by immediately throwing a formatted exception if the key is not present
 * in the container. This is useful for cases where the key is expected to exist
 * and a failure to find it indicates a programming error.
 *
...
template <typename Container, typename Key, typename... Args>
    requires AssociativeContainer<Container, Key>
typename Container::mapped_type findExc(
    const Container& container, const Key& key,
    fmt::format_string<Args...> fmt, Args&&... args) 
{
    return optional::unwrap(findOpt(container, key), fmt, std::forward<Args>(args)...);
}
