...
std::unique_ptr<Options> options::factory(int argc, const char** argv)
{
    ...
    app.add_flag("--disable-mirrors", opt.enableMirrors, "Disable mirror URLs");
    ...
    app.add_option("-l,--log-level", opt.logLevelInput,
        ...
        ->check(CLI::Range(1, 6));
    app.add_option(
        "-a,--answerfile", opt.answerfile, "Full path to an answerfile");
    app.add_option(
           "--skip", opt.skipSteps, "Skip specific steps during installation")
        ->multi_option_policy(CLI::MultiOptionPolicy::TakeAll);
    app.add_option("--force", opt.forceSteps,
           "Force specific steps during installation")
        ->multi_option_policy(CLI::MultiOptionPolicy::TakeAll);
    app.add_option("--stop-after", opt.stopAfterStep,
        "Stop after specific steps during installation");
...
#ifndef NDEBUG
    app.add_option(
        "--test", opt.testCommand, "Run a command for testing purposes");
    app.add_option(
           "--test-args", opt.testCommandArgs, "Arguments for test command")
        ->multi_option_policy(CLI::MultiOptionPolicy::TakeAll);
#endif
