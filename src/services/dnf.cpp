...
{
    return (cloyster::runCommand(fmt::format(
               "dnf -y install {}", package)) != 0);
}

bool dnf::remove(std::string_view package)
{
    return (cloyster::runCommand(fmt::format(
                "dnf -y remove {}", package)) != 0);
}

bool dnf::update(std::string_view package)
{
    return (cloyster::runCommand(fmt::format(
                "dnf -y update {}", package)) != 0);
}
...
void dnf::check()
{
    cloyster::runCommand("dnf check");
}

void dnf::clean()
{
    cloyster::runCommand("dnf clean all");
}
